...
const AccordionGallery = ({
  ...
  const handleEnter = (i: number) => {
    if (trigger === 'hover' && typeof window !== 'undefined' && window.matchMedia('(hover: hover)').matches) {
      setActive(i);
    }
  };
  ...
  const handleClick = (i: number, e: MouseEvent) => {
    if (lastClicked !== i) {
      ...
      // Second click on the same item, allow navigation
      setLastClicked(null);
    }
  };
  ...
  return (
    <div
      ...
      style={{ gap: `${gap}px`, height: vertical ? (typeof height === 'number' ? `${Math.round(height * 1.6)}px` : '600px') : (typeof height === 'number' ? `${height}px` : height) }}
      role="list"
            ...
            <span className="absolute inset-0 overflow-hidden [border-radius:inherit]">
              ...
              >
                <img
                  ...
                  draggable={false}
                  className="block h-full w-full select-none object-contain md:object-cover [-webkit-user-drag:none]"
                />
              </span>
              <span
